...
def records_to_graph():
    ...
    for i in movies:
        ...
        for j in ratxrange:
            t=i+"_"+j
            id1=id1+1
            mov1=Node(id1,t,'movie-rating')
            nodelist.append(mov1)
            nodedict[t]=mov1
    #print(nodedict)
    def looper(classy,c):
        global id1
    ...
    for i in ratings:
        ...
        t=ratings[i]
        for j in t:
            mov_rat_node=nodedict[j+'_'+t[j]]
            unode.neighbors.append(mov_rat_node)
            mov_rat_node.neighbors.append(unode)

    for i in movies:
        mnode=nodedict[i]
        t = movies[i]
        if(t.director!=None):
            dnode=nodedict[t.director]
            mnode.neighbors.append(dnode)
        ...
        def edit(classy):
            ...
            for j in classy:
                if classy== ratxrange:
                    j=i+'_'+j   
                tnode=nodedict[j]
                tnode.neighbors.append(mnode)
                mnode.neighbors.append(tnode)
