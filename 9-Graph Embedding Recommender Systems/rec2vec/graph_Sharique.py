...
def records_to_graph():
    ...
    for director in directors:
        node = Node(idx,director,'director')
        nodedict[director] = node
        nodelist.append(node)
        idx+=1
        
    for actor in actors:
        node = Node(idx,actor,'actor')
        nodedict[actor] = node
        nodelist.append(node)
        idx+=1
        
    for genre in genres:
        node = Node(idx,genre,'genre')
        nodedict[genre] = node
        nodelist.append(node)
        idx+=1
    ...
    # YOUR CODE HERE
    for movie in movies:
        node = Node(idx,movie,'movie')
        nodedict[movie] = node
        nodelist.append(node)
        idx+=1
        for i in range(0,6):
            ratingNode = Node(idx,movie+"_"+str(i),'rating')
            nodedict[movie+"_"+str(i)] = ratingNode
            nodelist.append(ratingNode)
            #get the movie node and add and edge between movie and its rating
            node.neighbors.append(ratingNode)
            ratingNode.neighbors.append(node)
            idx+=1
        #add node between movie and director
        movie = movies[movie]
        if(movie.director != None):
            directorNode = nodedict[movie.director]
            node.neighbors.append(directorNode)
            directorNode.neighbors.append(node)
        
        #add node between movie and actor
        for actor in movie.actors:
            actorNode = nodedict[actor]
            node.neighbors.append(actorNode)
            actorNode.neighbors.append(node)
        
        #add node between movie and genres
        for genre in movie.genres:
            genreNode = nodedict[genre]
            node.neighbors.append(genreNode)
            genreNode.neighbors.append(node)
        
    #print(movies)
    #add node between user and movie ratings
    for user in ratings:
        node = Node(idx,user,'user')
        nodedict[user] = node
        nodelist.append(node)
        idx+=1
        for m in ratings[user]:
            rating = ratings[user][m]
            ratingNode = nodedict[m+"_"+rating]
            ratingNode.neighbors.append(node)
            node.neighbors.append(ratingNode)
