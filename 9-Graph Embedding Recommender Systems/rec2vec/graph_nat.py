...
def records_to_graph():
    ...
    for movie in movies:
        n = Node(idx, movie, 'movie')
        nodedict[movie] = n
        nodelist.append(n)
        idx =idx+1
        for r in range(1,6):
            n = Node(idx, movie+"_"+str(r), 'rating')
            nodedict[movie+"_"+str(r)] = n
            nodelist.append(n)
            idx =idx+1

    for director in directors:
        n = Node(idx, director, 'director')
        nodedict[director] = n
        nodelist.append(n)
        idx =idx+1

    for actor in actors:
        n = Node(idx, actor, 'actor')
        nodedict[actor] = n
        nodelist.append(n)
        idx =idx+1

    for genre in genres:
        n = Node(idx, genre, 'genre')
        nodedict[genre] = n
        nodelist.append(n)
        idx =idx+1

    for user in ratings:
        n = Node(idx, user, 'user')
        nodedict[user] = n
        nodelist.append(n)
        idx =idx+1
    ...
    for i in ratings:
        ...
        userNode = nodedict[i]
        for movie in user:
            rnode = movie+"_"+user[movie]
            movieRating = nodedict[rnode]
            userNode.neighbors.append(movieRating)
            movieRating.neighbors.append(userNode)

    for i in movies:
        movie = movies[i]
        movieNode = nodedict[i]
        if (movie.director != None):
            directorNode = nodedict[movie.director]
            movieNode.neighbors.append(directorNode)
            directorNode.neighbors.append(movieNode)

        for j in movie.actors:
            actor = nodedict[j]
            actor.neighbors.append(movieNode)
            movieNode.neighbors.append(actor)

        for j in movie.genres:
            genre = nodedict[j]
            genre.neighbors.append(movieNode)
            movieNode.neighbors.append(genre)

        for j in range(1,6):
            rating = i+"_"+str(j)
            ratingNode = nodedict[rating]
