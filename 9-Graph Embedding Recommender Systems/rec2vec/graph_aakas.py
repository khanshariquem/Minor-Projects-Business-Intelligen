...
def records_to_graph():
    ...
    nodedict = {}
    # YOUR CODE HERE
    counter,ranger=0,['0','1','2','3','4','5']
   
    for i in ((movies)):
        temp=Node(counter,i,'movie')
        counter+=1
        nodelist.append(temp)
        nodedict[i]=temp
        for r in ranger:
            value=i+"_"+r
            temp2=Node(counter,value,'movie-rating')
            counter+=1
            nodelist.append(temp2)
            nodedict[value]=temp2
    
    for i in (genres):
        temp=Node(counter,i,'genre')
        counter+=1
        nodelist.append(temp)
        nodedict[i]=temp
        
    for i in (actors):
        temp=Node(counter,i,'actor')
        counter+=1
        nodelist.append(temp)
        nodedict[i]=temp
        
    for i in directors:
        temp=Node(counter,i,'director')
        counter+=1
        nodelist.append(temp)
        nodedict[i]=temp
    for i in ratings:
        temp=Node(counter,i,'user')
        counter+=1
        nodelist.append(temp)
        nodedict[i]=temp
    ...
    # YOUR CODE HERE
    for rr in ratings:
        current,cur_node = ratings[rr],nodedict[rr]
        for i in current:
            value = i+"_"+current[i]
            mn = nodedict[value]
            mn.neighbors.append(cur_node)
            cur_node.neighbors.append(mn)
    for movie in movies:
        current,current_node=movies[movie],nodedict[movie]
        if (current.director != None):
            dn=nodedict[current.director]
            dn.neighbors.append(current_node)
            current_node.neighbors.append(dn)
        for actor in current.actors:
            an=nodedict[actor]
            current_node.neighbors.append(an)
            an.neighbors.append(current_node)
        for rating in ranger:
            rn=nodedict[movie+"_"+rating]
            current_node.neighbors.append(rn)
            rn.neighbors.append(current_node)
        for topic in current.genres:
            gn=nodedict[topic]
            current_node.neighbors.append(gn)
            gn.neighbors.append(current_node)
